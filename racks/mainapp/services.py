    ...
    def get_devices_data(devices_report_qs: RawQuerySet) -> List[List[str]]:
        ...
        for device in devices_report_qs:
            devices_data.append([
                ...
                device.first_unit,
                device.last_unit,
                'Yes' if device.has_frontside_location else 'No',
                device.type,
                device.hostname,
                device.ip,
    ...
    def get_racks_data(racks_report_qs: RawQuerySet) -> List[List[str]]:
        ...
        for rack in racks_report_qs:
            racks_data.append([
                ...
                rack.model,
                rack.description,
                'Yes' if rack.has_numbering_from_bottom_to_top else 'No',
                rack.responsible,
                ...
                'Yes' if rack.has_external_ups else 'No',
                'Yes' if rack.has_cooler else 'No',
                rack.updated_by,
                rack.updated_at,
                rack.room_name,
                rack.building_name,
