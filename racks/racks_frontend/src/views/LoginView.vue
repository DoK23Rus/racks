    ...
    <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <template v-if="loginError != ''">
          <text class="text-xs text-red-500">{{ loginError }}</text>
        </template>
        <form @submit.prevent="submitForm" class="space-y-4 md:space-y-6">
          <div>
            <label for="username" class="block mb-2 text-sm text-gray-900 dark:text-white">
              Username:
            </label>
            <input v-model="username" type="username" name="username" id="e2e_username" class="bg-gray-50 border border-gray-300 
          ...
          <div>
            ...
            <input v-model="password" type="password" name="password" id="e2e_password" placeholder="••••••••" class="bg-gray-50 border border-gray-300
          ...
          <button type="submit" id="e2e_login" class="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-primary-300 
